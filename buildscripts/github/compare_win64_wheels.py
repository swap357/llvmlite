...
def unpack_wheel(wheel_path, output_dir):
    ...
    subprocess.run(["wheel", "unpack", wheel_path, "-d", output_dir], check=True)
    return output_dir
...
def compare_wheels(wheel1_path, wheel2_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if added_imports:
            print(f"Added imports: {added_imports}")
        if removed_imports:
            print(f"Removed imports: {removed_imports}")
        ...
        if added_delay:
            print(f"Added delay imports: {added_delay}")
        if removed_delay:
            print(f"Removed delay imports: {removed_delay}")
